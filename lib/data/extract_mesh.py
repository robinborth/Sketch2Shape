...
from lib.models.deepsdf import ActualDeepSDF, DeepSDF
...
# deepsdf = DeepSDF.load_from_checkpoint(
#     "/Users/korth/TUM/09/NiessnerLab/sketch2shape/checkpoint/deepsdf/last.ckpt"
# )

deepsdf = ActualDeepSDF.load_from_checkpoint(
    "/Users/korth/TUM/09/NiessnerLab/sketch2shape/checkpoint/deepsdf/shapenet_chair_single-epoch=4999-train/loss=0.00.ckpt"
)
...
lat_vec = deepsdf.lat_vecs(lat_vec)
x = (xyz, lat_vec)
sd = deepsdf((xyz.unsqueeze(0), lat_vec.unsqueeze(0)))
# sd = deepsdf.predict(xyz)
sd_r = sd.reshape(2 * N, 2 * N, 2 * N).detach().numpy()
