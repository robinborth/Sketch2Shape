...
@hydra.main(version_base=None, config_path="../conf", config_name="eval_deepsdf")
def evaluate(cfg: DictConfig) -> None:
    log.info("==> loading config ...")
    L.seed_everything(cfg.seed)

    log.info(
        f"==> initializing datamodule <{cfg.data_train._target_}> with data {cfg.experiment}"
    )
    datamodule_train: LightningDataModule = hydra.utils.instantiate(cfg.data_train)
    log.info(
